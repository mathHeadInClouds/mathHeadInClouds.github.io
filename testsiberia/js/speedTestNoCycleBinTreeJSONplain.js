...
function create_test(serialize_func, deserialize_func){
	return function(depth){
	    var inputTree = completeBinaryTree(depth);
	    var start = performance.now();
	    var aString = serialize_func(inputTree);
	    var end1 = performance.now();
	    var duration1 = Math.round(1000*(end1 - start))/1000;
	    var length = aString.length;
	    start = performance.now();
	    var roundTrip = deserialize_func(aString);
...
var siberianTable, plainTable, ratiosTable;
var siberianMaxDepth = 20;
var plainMaxDepth = 20;

function runSpeedTest(){
	var body = document.body;
	var siberianTest = create_test(JSON.Siberia.serialize, JSON.Siberia.unserialize);
	var plainTest = create_test(JSON.stringify, JSON.parse);
	siberianTable = table(
		caption('siberia'),
		tr(th('depth')),
		tr(th('duration serialize (ms)')),
		tr(th('string length')),
		tr(th('duration deserialize (ms)'))
	);
	plainTable = table(
		caption("plain JSON.stringify/.parse"),
		tr(th('depth')),
		tr(th('duration serialize (ms)')),
		tr(th('string length')),
		tr(th('duration deserialize (ms)'))
	...
	ratiosTable = table(
		caption("ratios"),
		tr(th('depth')),
		tr(th('serialize ratio')),
		tr(th('deserialize ratio')),
		tr(th('round trip ratio')),
		tr(th('string length ratio'))
	);
	body.appendChild(h1('complete binary tree - no cycles, directed or undirected - Siberia vs plain JSON'));
	...
	body.appendChild(hr());
	body.appendChild(plainTable);
	for (var depth=1; depth<plainMaxDepth; depth++){
		Tasks.push(new Task(plainTest, [depth], function(testResult){
			plainTable.childNodes[1].appendChild(th(testResult[0]));
			plainTable.childNodes[2].appendChild(td(testResult[1]));
			plainTable.childNodes[3].appendChild(td(testResult[2]));
			plainTable.childNodes[4].appendChild(td(testResult[3]));
		}));
	}
	body.appendChild(hr());
	executeNextTask();
...
function afterDone(){
	var body = document.body;
	var maxDepth = Math.min(siberianMaxDepth, plainMaxDepth);
	for (var depth=1; depth<maxDepth; depth++){
		var duration1_siberian     = +siberianTable.childNodes[2].childNodes[depth].textContent;
		var duration1_crockfordian = +plainTable.childNodes[2].childNodes[depth].textContent;
		var duration2_siberian     = +siberianTable.childNodes[4].childNodes[depth].textContent;
		var duration2_crockfordian = +plainTable.childNodes[4].childNodes[depth].textContent;
		var strLength_siberian     = +siberianTable.childNodes[3].childNodes[depth].textContent;
		var strLength_crockfordian = +plainTable.childNodes[3].childNodes[depth].textContent;
