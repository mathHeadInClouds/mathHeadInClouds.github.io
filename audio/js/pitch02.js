...
function autoCorrelate( buf, sampleRate ) {
	...
	for (var offset = 15; offset < MAX_SAMPLES; offset++) {
		...
		for (var i=0; i<MAX_SAMPLES; i++) {
			var diff = buf[i] - buf[i+offset];
			_correlation += Math.abs(diff);
		}
	...
	if (best_correlation > 0.01) {
		...
		var returnValue = sampleRate/best_offset;
		spanElt.textContent = [loopCounter, best_offset, best_correlation, returnValue].map(round1000th).join(' ');
		return returnValue;
	}
...
function updatePitch( time ) {
	...
	var ac = autoCorrelate( buf, audioContext.sampleRate );
 	if (ac == -1) {
		...
		console.log(loopCounter, Math.round(pitch) + 'Hz', noteElem.innerHTML);
		if (detune == 0 ) {
			detuneElem.className = "";
