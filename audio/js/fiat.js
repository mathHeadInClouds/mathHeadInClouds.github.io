...
(function(root){
    var generateColor = (function(){
        ...
        var brightnessFactor = 0.75; // positive floating point number; the larger the brighter; for dark colors, make it less than 1
...
result.xy_vars = function(){
    ...
    return retval;
        ...
        result.set_or_add_root = function(multi_instance){
            ...
            if (multi_instance){
                $name = arguments[1];
                // lib.$$name = lib.$$name || []; lib.$$name.push($name);
            } else {
                if (lib.instance){
    ...
    UTIL.dom.setParentAndViz = function(node, parent, visibility){
    ...
    UTIL.dom.setOrRemoveAttribute = function(nodeArg, attName, attValue, theFlag){
        var node = domNodeOfElement(nodeArg);
        if (theFlag){
