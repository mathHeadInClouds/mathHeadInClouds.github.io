...
function create_iffalse_statement(allVarsObj){
	...
	return 'if(false) { console.log(' + allVarsArray.join(', ') + '); }';
}

function generateAllVarsObject(node, inScope, colorize){
	var clickedName = '';
	var foundAt = null;
...
function coloringAction(elt, evt, verbose){
	...
	var allVarsObj = generateAllVarsObject(clickedAstNode, inScope, true);
	if (verbose){
		/*CONSOLE.textContent =  varNamesArray2D.map(function(varNames){
			return varNames.join(', ');
		}).join('\n');*/
		var stmt = create_iffalse_statement(allVarsObj);
...
function initCodeContainer(sourceString, attachClickHandler, injectCode){
	...
	if (injectCode){
		estraverse.traverse(ast, {
			enter: function(node, parent){
				if (parent){
					var isFunc = (parent.type==='FunctionDeclaration') || (parent.type==='FunctionExpression');
					var isBlock = (node.type==='BlockStatement');
					if (isFunc && isBlock){
						var bodyStartIdx = node.range[0];
						var currentSpan = codeSpans[bodyStartIdx];
						var nextSpan = currentSpan.nextElementSibling;
						var inScope = inScopeAt(bodyStartIdx);
						var allVarsObj = generateAllVarsObject(node, inScope, false);
						var statement = create_iffalse_statement(allVarsObj);
						codeContainer.insertBefore(document.createTextNode(statement), nextSpan);
					}
				}
			}
		});
...
function iffalse_everywhere(){
	clickAnywhere.style.visibility = 'hidden';
	clearCodeContainer();
	clearSwatches();
	clearConsole();
	initCodeContainer(theTextArea.value, false, true);
}
function performAnalysis(){
	clickAnywhere.style.visibility = '';
...
function bodyOnload(){
	...
	clickAnywhere  = document.getElementById('clickAnywhere');
	buttonOne      = document.getElementById('buttonOne');
	buttonAll      = document.getElementById('buttonAll');
	buttonOne.addEventListener('click', performAnalysis);
	buttonAll.addEventListener('click', iffalse_everywhere);
